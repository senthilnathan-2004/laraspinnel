    ...
    <section className="relative overflow-hidden py-20 bg-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-6 space-y-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-brand-border pb-4 gap-3 sm:gap-0">
          ...
          <Link
            href="/gallery"
            className="group inline-flex items-center gap-1 text-sm font-semibold text-goat-primary hover:text-goat-hover transition-colors whitespace-nowrap"
          >
            <span>View All</span>
          ...
          {items.map((img, i) => {
            ...
            return (
              <Link
                href="/gallery"
                key={img._id}
                className={`relative w-full h-full overflow-hidden rounded-[1.5rem] md:rounded-[2rem] border-4 bg-brand-light-gray group cursor-pointer transition-all duration-500 ease-out ${isActive
